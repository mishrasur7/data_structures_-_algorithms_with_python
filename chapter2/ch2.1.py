...
class IntArray():
    ...
    def insert(self, index:int, val:int) -> None:
        # check index and val
        if not isinstance(index, int) or not self._min_val <= val <= self._max_val:
        ...
        #allocate new memory location with new size
        new_resmem = ReservedMemory(self._size * self._bytes_per_element)

        # copy the old reserved momory area
        if index > 0:
            new_resmem.copy(self._resmem, count=index * self._bytes_per_element)
        
          # Convert the value to be stored (accounting for shifting)
        val_to_store = val + self._shift_val

        # Insert new value into new_resmem at the correct position
        for byte_index in range(self._bytes_per_element):
        ...
        if index < self._size - 1:  # If the new value is not being inserted at the end
            new_resmem.copy(
                self._resmem,
